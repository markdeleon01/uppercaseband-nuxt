...
describe('Discography.vue', () => {
  ...
  it('ensures the Discography page is rendered containing key sections', () => {
    ...
    expect(wrapper.find('.discography').exists()).toBeTruthy()
    expect(wrapper.findAll('.release-item').length).toEqual(6)
    expect(wrapper.findAll('.release-thumbnail').length).toEqual(6)
    expect(wrapper.findAll('.release-item a').length).toEqual(6)
  })
})
